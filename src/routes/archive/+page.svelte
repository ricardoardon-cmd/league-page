    ...
    <section class="videoSection">
            ...
            <div class="videoStage">
                <div class="videoEmbed">
                    <iframe
                        ...
                        title={`GGL 2025 ${currentVideo.title}`}
                        frameborder="0"
        ...
        <div class="videoDots" aria-label="Choose a video">
            {#each videos as video, index}
                <button class:active={activeVideo === index} onclick={() => activeVideo = index} aria-label={`Play ${video.title}`}></button>
            {/each}
        </div>
    ...
    <section class="gallerySection">
        ...
        <div class="gallery">
            {#each visibleMoments as moment}
                <figure class:featured={moment.featured}>
                    <img src={moment.src} alt={moment.title} />
                    <figcaption><span>{moment.tag}</span><strong>{moment.title}</strong></figcaption>
                </figure>
            {/each}
        </div>
        {#if visibleMoments.length === 0}
            <div class="comingSoon">The rest of the 2025 photos are being added to this category.</div>
        {/if}
    </section>
