<script>
    ...
    const createRoom = async () => {
        ...
        try {
            ...
            const randomSlots = shuffle(Array.from({ length: 10 }, (_, i) => i + 1));
            const teamRows = randomSlots.map((_, index) => ({
                room_id: room.id,
                draft_slot: index + 1,
                is_cpu: true
            }));
    ...
    const joinRoom = async () => {
        ...
        try {
            ...
            const rows = await request(`mock_rooms?room_code=eq.${encodeURIComponent(code)}&select=*`);
...
<div class="page">
    ...
    {#if mode === 'home'}
        <section class="card homeCard">
            ...
            <button class="primary" onclick={createRoom} disabled={loading}>
                {loading ? 'Creating room…' : 'Create Mock Draft'}
            </button>

            <div class="divider"><span>or join a room</span></div>

            <div class="joinRow">
                <input bind:value={roomCode} oninput={() => roomCode = roomCode.toUpperCase()} maxlength="6" placeholder="ROOM CODE" />
                <button onclick={joinRoom} disabled={loading}>Join</button>
            </div>

            <div class="featureGrid">
                <div><b>10 teams</b><span>Random draft order</span></div>
    ...
    {:else if room}
        ...
        <div class="sectionTitle">
            <div>
                <h2>Draft Order</h2>
                <p>Tap any CPU slot to claim it. Everyone in the room sees changes automatically.</p>
            </div>
