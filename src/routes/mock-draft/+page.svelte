<script>
    ...
    const refreshRoom = async () => {
        ...
        try {
            ...
            if (room?.status === 'drafting') {
                requestAnimationFrame(maybeRunCpu);
            }
        } catch (err) {
            console.error(err);
        }
    ...
    function pickContext(overallPick = room?.current_pick || 1) {
        const teamCount = room?.team_count || 10;
        const round = Math.floor((overallPick - 1) / teamCount) + 1;
        const index = (overallPick - 1) % teamCount;
        const slot = round % 2 === 1 ? index + 1 : teamCount - index;
        const team = teams.find((item) => item.draft_slot === slot);
        return { overallPick, round, pickInRound: index + 1, slot, team };
    }
    ...
    async function makePick(player, team = current?.team) {
        ...
        const context = pickContext(room.current_pick);
        if (context.team?.id !== team.id) return;

        const pos = normalizePosition(player.pos || player.position);
        const totalPicks = (room.team_count || 10) * (room.rounds || 16);
...
<div class="page">
    ...
    {:else if mode === 'draft' && room}
        <section class="draftTop">
            ...
            <div><span>{room.status === 'completed' ? 'Draft complete' : `Round ${current?.round} · Pick ${current?.overallPick}`}</span><strong>{room.status === 'completed' ? 'GGL Mock Complete' : `${current?.team?.is_cpu ? 'GGL CPU' : current?.team?.manager_name || 'Team'} is on the clock`}</strong></div>
        ...
        <div class="draftLayout">
            <section class="boardPanel">
                <div class="panelHeading"><h2>Draft Board</h2><span>{picks.length}/{(room.team_count || 10) * (room.rounds || 16)}</span></div>
                <div class="boardScroller">
                    <div class="board">
                        {#each Array.from({length: room.rounds || 16}, (_, r) => r + 1) as round}
                            <div class="roundLabel">R{round}</div>
                            {#each Array.from({length: room.team_count || 10}, (_, i) => i + 1) as pickNo}
                                {@const overall = (round - 1) * (room.team_count || 10) + pickNo}
                                {@const slot = round % 2 === 1 ? pickNo : (room.team_count || 10) - pickNo + 1}
                                {@const team = teams.find((item) => item.draft_slot === slot)}
                                ...
                                <div class:activePick={room.current_pick === overall && room.status === 'drafting'} class:minePick={team?.player_token === clientToken} class={`boardCell ${pick ? `pos-${pick.position}` : ''}`}>
                                    <small>{round}.{String(pickNo).padStart(2,'0')} · S{slot}</small>
                                    {#if pick}<b>{pick.player_name}</b><span>{pick.position}</span>{:else}<b>{team?.is_cpu ? 'CPU' : team?.manager_name || `Slot ${slot}`}</b><span>—</span>{/if}
                                </div>
                            {/each}
                        {/each}
                    </div>
