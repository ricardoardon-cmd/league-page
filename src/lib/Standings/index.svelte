<script>
    ...
    export let playoffMatchupsData;
    ...
    let playoffBracket = [];
    let bracketRounds = [];
    let playoffMatchups = { byRound: {}, weeksByRound: {} };
    let players = {};
    let openBracketMatchup = null;

    const buildPreseasonStandings = (teamManagers, season) => {
        const seasonTeams = teamManagers?.teamManagersMap?.[season] || {};
        return Object.keys(seasonTeams).map((rosterID) => ({
    ...
    const getBracketMatchupKey = (round, matchup) => `${round}-${matchup?.m ?? matchup?.p ?? 'match'}`;

    const getSleeperMatchup = (round, bracketMatchup) => {
        const roundMatchups = playoffMatchups?.byRound?.[round] || [];
        const rosterOne = normalizeRosterID(bracketMatchup?.t1);
        const rosterTwo = normalizeRosterID(bracketMatchup?.t2);
        if(!rosterOne || !rosterTwo) return null;

        const entryOne = roundMatchups.find((entry) => Number(entry?.roster_id) === rosterOne);
        const entryTwo = roundMatchups.find((entry) => Number(entry?.roster_id) === rosterTwo);
        if(!entryOne || !entryTwo) return null;
        if(entryOne.matchup_id == null || entryTwo.matchup_id == null || Number(entryOne.matchup_id) !== Number(entryTwo.matchup_id)) return null;
        return [entryOne, entryTwo];
    };

    const toggleBracketMatchup = (round, matchup) => {
        if(!getSleeperMatchup(round, matchup)) return;
        const key = getBracketMatchupKey(round, matchup);
        openBracketMatchup = openBracketMatchup === key ? null : key;
    };

    const handleBracketKey = (event, round, matchup) => {
        if(event.key === 'Enter' || event.key === ' ') {
            event.preventDefault();
            toggleBracketMatchup(round, matchup);
        }
    };
    ...
    onMount(async () => {
        ...
        [playoffBracket, playoffMatchups, players] = await Promise.all([
            playoffBracketData,
            playoffMatchupsData,
            playersData
        ]);
        playoffBracket = playoffBracket || [];
        playoffMatchups = playoffMatchups || { byRound:{}, weeksByRound:{} };
        players = players || {};
...
<style>
    ...
    .standingsHeader p { margin:10px 0 0; opacity:.65; }
    .seasonControls { display:flex; justify-content:center; align-items:center; gap:10px; flex-wrap:wrap; margin-top:14px; }
    .seasonSelect { appearance:auto; min-width:105px; padding:7px 11px; border-radius:999px; border:1px solid var(--ccc); background:var(--fff); color:inherit; font:inherit; font-size:.78rem; font-weight:800; cursor:pointer; }
    .seasonStatus { display:inline-flex; align-items:center; gap:7px; padding:7px 13px; border-radius:999px; background:var(--f3f3f3); border:1px solid var(--ccc); font-size:.72rem; font-weight:800; letter-spacing:.4px; text-transform:uppercase; }
    ...
    .viewButton { border:0; border-radius:999px; padding:9px 18px; background:transparent; color:inherit; font:inherit; font-size:.78rem; font-weight:800; cursor:pointer; transition:background .15s ease,box-shadow .15s ease; }
    .viewButtonActive { background:var(--fff); box-shadow:0 2px 8px rgba(0,0,0,.08); }
    .loadingCard { width:95%; max-width:700px; margin:35px auto; padding:38px 28px; box-sizing:border-box; text-align:center; border-radius:20px; background:var(--fff); border:1px solid var(--ccc); box-shadow:0 4px 16px rgba(0,0,0,.07); }
    .loadingBar { width:85%; max-width:450px; margin:18px auto 0; }
    .preseasonMessage { width:100%; box-sizing:border-box; margin:0 0 14px; padding:12px 16px; border-radius:12px; background:var(--f3f3f3); border:1px solid var(--ccc); text-align:center; font-size:.82rem; font-weight:650; opacity:.8; }
    .standingsCard { width:100%; box-sizing:border-box; padding:16px; border-radius:18px; background:var(--fff); border:1px solid var(--ccc); box-shadow:0 4px 16px rgba(0,0,0,.07); overflow:hidden; }
    .columns { display:grid; grid-template-columns:60px minmax(240px,1fr) 120px 110px 110px 100px; align-items:center; gap:10px; padding:0 16px 10px; font-size:.68rem; font-weight:800; letter-spacing:.55px; text-transform:uppercase; opacity:.55; }
    .center { text-align:center; } .list { width:100%; }
    .bracketEmpty { width:95%; max-width:700px; margin:20px auto 0; padding:42px 28px; box-sizing:border-box; text-align:center; border-radius:20px; background:var(--fff); border:1px solid var(--ccc); box-shadow:0 4px 16px rgba(0,0,0,.07); }
    ...
    .bracketEmpty p { max-width:500px; margin:10px auto 0; line-height:1.55; opacity:.65; }
    .bracketShell { width:100%; overflow-x:auto; padding:4px 2px 18px; box-sizing:border-box; }
    .championBanner { max-width:620px; margin:0 auto 22px; padding:16px 20px; border-radius:16px; background:var(--fff); border:1px solid var(--ccc); box-shadow:0 4px 16px rgba(0,0,0,.07); text-align:center; }
    .championEyebrow { font-size:.7rem; font-weight:800; letter-spacing:1px; text-transform:uppercase; opacity:.55; }
    ...
    .roundColumn { min-width:0; } .roundTitle { margin-bottom:12px; text-align:center; font-size:.76rem; font-weight:850; letter-spacing:.8px; text-transform:uppercase; opacity:.6; }
    .roundMatchups { display:flex; flex-direction:column; gap:16px; }
    .bracketMatchup { padding:12px; border-radius:16px; background:var(--fff); border:1px solid var(--ccc); box-shadow:0 4px 14px rgba(0,0,0,.06); }
    .bracketMatchupClickable { cursor:pointer; transition:transform .15s ease,box-shadow .15s ease; }
    .bracketMatchupClickable:hover { transform:translateY(-1px); box-shadow:0 6px 18px rgba(0,0,0,.1); }
    .matchupLabel { margin-bottom:9px; font-size:.66rem; font-weight:800; letter-spacing:.55px; text-transform:uppercase; opacity:.5; }
    .matchupHint { margin-top:9px; font-size:.62rem; font-weight:800; letter-spacing:.5px; text-transform:uppercase; opacity:.45; }
    ...
    .bracketTeam + .bracketTeam { margin-top:7px; } .bracketWinner { border-color:#d4af37; font-weight:800; } .bracketLoser { opacity:.55; }
    .bracketAvatar { width:34px; height:34px; border-radius:50%; object-fit:cover; flex-shrink:0; }
    .bracketTeamInfo { min-width:0; text-align:left; } .bracketTeamName { font-size:.82rem; font-weight:800; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .bracketManager { margin-top:2px; font-size:.68rem; opacity:.55; } .bracketTbd { font-size:.75rem; font-weight:700; opacity:.55; } .winnerMark { margin-left:auto; font-size:.9rem; }
    .expandedMatchup { grid-column:1 / -1; width:100%; max-width:680px; margin:4px auto 10px; }
    .expandedTitle { margin:0 0 8px; font-size:.72rem; font-weight:850; letter-spacing:.7px; text-transform:uppercase; opacity:.6; text-align:center; }
    @media(max-width:800px) {
        .standingsPage { padding:20px 10px 50px; } .standingsHeader h1 { font-size:2rem; } .standingsCard { padding:10px; }
        .columns { grid-template-columns:42px 1fr auto; grid-template-areas:'rank team record'; padding:0 14px 9px; }
        .columnRank { grid-area:rank; } .columnTeam { grid-area:team; } .columnRecord { grid-area:record; text-align:right; } .columnDesktop { display:none; }
        .viewButton { padding:8px 14px; font-size:.72rem; } .bracketEmpty { padding:32px 18px; } .bracket { min-width:820px; grid-auto-columns:240px; gap:16px; }
    }
</style>
...
<div class="standingsPage">
            ...
            <div class="seasonControls">
                {#if availableSeasons.length > 1}
                    <select class="seasonSelect" aria-label="Select season" value={selectedSeason} onchange={changeSeason}>
                        {#each availableSeasons as season}<option value={season}>{season}</option>{/each}
                    </select>
                {/if}
                {#if preseason}
    ...
    {:else if activeView === 'standings'}
        ...
        <div class="standingsCard">
            <div class="columns"><div class="columnRank center">#</div><div class="columnTeam">Team</div><div class="columnRecord center">Record</div><div class="columnDesktop center">PF</div><div class="columnDesktop center">PA</div><div class="columnDesktop center">Streak</div></div>
            <div class="list">
                {#each standings as standing,ix}<Standing {standing} {leagueTeamManagers} preseason={preseason} rank={ix+1} team={getTeamFromTeamManagers(leagueTeamManagers,standing.rosterID,year)} />{/each}
            </div>
        </div>
    {:else}
        {#if !playoffBracket.length}
            <div class="bracketEmpty"><div class="bracketEmptyIcon">🏆</div><h2>Playoff bracket coming soon</h2><p>Sleeper has not seeded the {year || ''} playoff field yet. Once the bracket is created, the teams and advancement paths will appear here automatically.</p></div>
        {:else}
                    ...
                    <section class="roundColumn">
                        ...
                        <div class="roundMatchups">
                            {#each bracketRound.matchups as matchup}
                                ...
                                {@const sleeperMatchup = getSleeperMatchup(bracketRound.round,matchup)}
                                {@const matchupKey = getBracketMatchupKey(bracketRound.round,matchup)}
                                <div
                                    class:bracketMatchupClickable={!!sleeperMatchup}
                                    class="bracketMatchup"
                                    role={sleeperMatchup ? 'button' : undefined}
                                    tabindex={sleeperMatchup ? '0' : undefined}
                                    onclick={() => toggleBracketMatchup(bracketRound.round,matchup)}
                                    onkeydown={(event) => handleBracketKey(event,bracketRound.round,matchup)}
                                >
                                    ...
                                    {#if sleeperMatchup}<div class="matchupHint">{openBracketMatchup === matchupKey ? 'Tap to close lineup' : 'Tap to view lineup'}</div>{/if}
                                </div>

                                {#if sleeperMatchup && openBracketMatchup === matchupKey}
                                    <div class="expandedMatchup" onclick={(event) => event.stopPropagation()}>
                                        <div class="expandedTitle">Week {playoffMatchups?.weeksByRound?.[bracketRound.round]} · Full Matchup</div>
                                        <Matchup
                                            matchup={sleeperMatchup}
                                            {players}
                                            active={matchupKey}
                                            ix={matchupKey}
                                            displayWeek={playoffMatchups?.weeksByRound?.[bracketRound.round]}
                                            expandOverride={true}
