...
<style>
    ...
    @media (max-width: 700px) {
        ...
        .category-scoring :global(.rankingHolder), .category-scoring :global(.rankingTableWrapper),
        .category-scoring :global(.buttonHolder), .category-scoring :global(h4) { display: none !important; }

        .category-season :global(.scoringHighAnchor), .category-season :global(.scoringLowAnchor),
        .category-season :global(.blowoutAnchor), .category-season :global(.closestAnchor),
        .category-season :global(.rankingHolder), .category-season :global(.rankingTableWrapper),
        .category-season :global(.buttonHolder), .category-season :global(h4) { display: none !important; }

        .category-matchups :global(.scoringHighAnchor), .category-matchups :global(.scoringLowAnchor),
        .category-matchups :global(.seasonHighTable), .category-matchups :global(.seasonLowTable),
        .category-matchups :global(.rankingHolder), .category-matchups :global(.rankingTableWrapper),
        .category-matchups :global(.buttonHolder), .category-matchups :global(h4) { display: none !important; }
    ...
    @media (max-width: 415px) { :global(.buttonHolder .selectionButtons) { font-size: 0.5em; padding: 0 6px; } }
    @media (max-width: 315px) { :global(.buttonHolder .selectionButtons) { font-size: 0.45em; padding: 0 3px; } }
</style>

<div class="rankingsWrapper">
    <div class="buttonHolder">
        <Group variant="outlined">
            <Button class="selectionButtons" onclick={() => key = "regularSeasonData"} variant="{key == "regularSeasonData" ? "raised" : "outlined"}"><Label>Regular Season</Label></Button>
            <Button class="selectionButtons" onclick={() => key = "playoffData"} variant="{key == "playoffData" ? "raised" : "outlined"}"><Label>Playoffs</Label></Button>
        </Group>
        <br />
        <Group variant="outlined">
            <Button class="selectionButtons" onclick={() => setDisplay("allTime")} variant="{display == "allTime" ? "raised" : "outlined"}"><Label>All-Time Records</Label></Button>
            <Button class="selectionButtons" onclick={() => setDisplay("season")} variant="{display == "season" ? "raised" : "outlined"}"><Label>Season Records</Label></Button>
        </Group>
    </div>
    ...
    <div class="recordsContent category-{mobileCategory}">
        {#if display == "allTime"}
            ...
            {:else}<p class="empty">No records <i>yet</i>...</p>{/if}
        {:else}
