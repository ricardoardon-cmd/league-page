...
export const getLeagueTeamManagers = async () => {
    ...
    let teamManagersMap = {};
    let leagueIDsBySeason = {};
    let finalUsers = {};
    let currentSeason = null;

    // Loop through every connected Sleeper season and create
    // [year][roster_id]: team/managers plus a year -> league ID lookup.
    while(currentLeagueID && currentLeagueID != 0) {
        const seasonLeagueID = String(currentLeagueID);
        const [usersRaw, leagueData, rostersRaw] = await waitForAll(
            fetch(`https://api.sleeper.app/v1/league/${seasonLeagueID}/users`, {compress: true}),
            getLeagueData(seasonLeagueID),
            fetch(`https://api.sleeper.app/v1/league/${seasonLeagueID}/rosters`, {compress: true}),
        ...
        leagueIDsBySeason[year] = seasonLeagueID;
