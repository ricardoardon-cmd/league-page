...
<style>
	...
	.S-FLEX { background: linear-gradient(to right, var(--WR), var(--WR) 25%, var(--RB) 25%, var(--RB) 50%, var(--TE) 50%, var(--TE) 75%, var(--QB)); background-color: #8f66ff; }
	.R-FLEX { background: linear-gradient(to right, var(--WR), var(--WR) 50%, var(--TE) 50%); }
	...
	:global(.slot) { width: 22px !important; padding: 1px 6px 1px 8px !important; }
	:global(.avatar) { width: 45px; }
	:global(.playerCell) { height: 45px !important; overflow-y: hidden !important; }
	:global(.nameCell) { padding: 0 5px; }

	.clickablePlayer { cursor: pointer; }
	.clickablePlayer:hover .info { text-decoration: underline; text-decoration-color: var(--blueOne); text-underline-offset: 3px; }

	span.nickname { color: #888; font-style: italic; font-size: 0.8em; }
	:global(.injury) { font-style: italic; font-weight: 700; font-size: 0.7em; margin-left: 0.5em; vertical-align:super; }
	:global(.Q) { color: #c09111; }
	:global(.OUT), :global(.IR), :global(.PUP) { color: #851313; }
    .additionalInfo { display: inline-block; font-size: 0.7em; }
	...
	@media (max-width: 460px) {
		:global(.nameCell) { font-size: 0.9em; }
	}

    @media (max-width: 355px) {
        :global(.slot) { width: 22px !important; padding: 1px 2px 1px 2px !important; }
        :global(.avatar) { padding: 0px 16px 0px 2px !important; }
        .pos { width: 42px; height: 24px; font-size: 0.8em; }
		:global(.nameCell) { font-size: 0.85em; }
    }
</style>
