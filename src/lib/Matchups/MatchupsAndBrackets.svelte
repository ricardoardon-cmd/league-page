    ...
    const groupMatchups = (rawMatchups = []) => {
        ...
        for (const match of rawMatchups || []) {
            ...
            grouped[match.matchup_id].push({
                ...
                player_points: match.players_points || {}
            });
        }
...
<style>
    .matchupCenter { width: 100%; max-width: 1250px; margin: 0 auto; padding: 30px 20px 70px; box-sizing: border-box; }
    .pageHeader { text-align: center; margin-bottom: 22px; }
    .pageHeader h1 { margin: 0; font-size: 2.5rem; font-weight: 800; letter-spacing: -0.5px; }
    .pageHeader p { margin: 8px 0 0; opacity: 0.7; font-size: 1rem; }
    .yearFilter { display: flex; justify-content: center; align-items: center; gap: 10px; width: fit-content; margin: 0 auto 18px; padding: 9px 12px; border-radius: 14px; background: var(--f3f3f3); box-shadow: 0 3px 14px rgba(0,0,0,0.07); }
    .yearFilter label { font-size: 0.78rem; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; opacity: 0.65; }
    .yearSelect { min-width: 100px; padding: 8px 30px 8px 10px; border: 1px solid var(--ccc); border-radius: 9px; background: var(--fff); color: inherit; font: inherit; font-weight: 750; cursor: pointer; }
    .seasonControls { display: flex; flex-direction: column; align-items: center; gap: 12px; margin-bottom: 20px; }
    .controlCard { display: flex; justify-content: center; align-items: center; flex-wrap: wrap; gap: 10px; padding: 10px; border-radius: 14px; background: var(--f3f3f3); box-shadow: 0 3px 14px rgba(0,0,0,0.07); }
    .bracketControls { display: flex; justify-content: center; flex-wrap: wrap; gap: 8px; }
    .message { display: block; width: 85%; max-width: 500px; margin: 50px auto 80px; text-align: center; padding: 25px; border-radius: 14px; background: var(--f3f3f3); }
    .emptyTitle { font-size: 1.5rem; font-weight: 750; margin-bottom: 8px; }
    .emptyText { opacity: 0.7; }

    @media (max-width: 600px) {
        .matchupCenter { padding: 20px 10px 50px; }
        .pageHeader h1 { font-size: 2rem; }
        .controlCard { width: 100%; box-sizing: border-box; }
        .yearFilter { width: 100%; box-sizing: border-box; }
    }
</style>
...
<div class="matchupCenter">
    {#if loading}
        <div class="message">
            <div class="emptyTitle">🏈 Matchup Center</div>
            <p class="emptyText">Loading league matchups...</p>
            <LinearProgress indeterminate />
        </div>
    {:else}
        <div class="pageHeader">
            <h1>🏈 Matchup Center</h1>
            <p>{year} GGL Fantasy Football</p>
        </div>

        {#if seasonOptions.length > 1}
            <div class="yearFilter">
                <label for="matchup-year">Season</label>
                <select id="matchup-year" class="yearSelect" value={selectedYear} onchange={changeSeason} disabled={seasonLoading}>
                    ...
                    <div class="controlCard">
                        <Group variant="outlined">
                            ...
                            <Button class="selectionButtons" onclick={() => changeSelection('champions')} variant={selection == 'champions' || selection == 'losers' ? 'raised' : 'outlined'}>
                                <Label>Playoff Brackets</Label>
                            </Button>
                        </Group>
                    </div>
                {/if}

                {#if Number(selectedYear) === Number(currentSeason) && (selection == 'champions' || selection == 'losers')}
                    <div class="controlCard bracketControls">
                        <Group variant="outlined">
                            <Button class="selectionButtons" onclick={() => changeSelection('champions')} variant={selection == 'champions' ? 'raised' : 'outlined'}>
                                <Label>Championship Bracket</Label>
                            </Button>
        ...
        {:else}
            <div class="message">
                <div class="emptyTitle">🏈 No Matchups Yet</div>
                <p class="emptyText">No matchup data is available for {year}.</p>
            </div>
        {/if}

        {#if Number(selectedYear) === Number(currentSeason) && brackets?.champs?.bracket?.[0]?.[0]?.[0]?.points && (selection == 'champions' || selection == 'losers')}
            <Brackets {queryWeek} {leagueTeamManagers} {players} {brackets} bind:selection />
        {/if}
    {/if}
</div>
