<script>
    ...
    const manager = (id) => (managers || []).find((item) => Number(item?.roster) === Number(id)) || { name: team(id).name };
    const emphasizeManagers = (text = '') => {
        let output = String(text);
        const names = [...new Set((managers || []).map((item) => item?.name).filter(Boolean))].sort((a, b) => b.length - a.length);
        for (const name of names) output = output.split(name).join(`<strong>${name}</strong>`);
        return output;
    };
    ...
    const build = () => {
        ...
        for (const matchup of matchupArray || []) {
            ...
            const A = manager(a.roster_id), B = manager(b.roster_id), aHistory = hist(a.roster_id), bHistory = hist(b.roster_id), aRank = st.find((entry) => entry.id === Number(a.roster_id))?.rank || 99, bRank = st.find((entry) => entry.id === Number(b.roster_id))?.rank || 99, aProjection = n(proj(a)), bProjection = n(proj(b)), meeting = priorMeeting(a.roster_id, b.roster_id), injuryText = matchupInjurySentence(a, b, players, displayWeek, A.name, B.name, 'pregame');
            ...
            const hotCold = (aHistory.type === 'W' && aHistory.streak >= 2 && bHistory.type === 'L' && bHistory.streak >= 2) || (bHistory.type === 'W' && bHistory.streak >= 2 && aHistory.type === 'L' && aHistory.streak >= 2); if (hotCold) { const hot = aHistory.type === 'W' ? A : B, cold = aHistory.type === 'L' ? A : B, hotStreak = aHistory.type === 'W' ? aHistory.streak : bHistory.streak, coldStreak = aHistory.type === 'L' ? aHistory.streak : bHistory.streak; stories.push({ p: 7, label: '📈 Hot vs. Cold', text: `${hot.name} comes in riding a ${hotStreak}-game winning streak while ${cold.name} has dropped ${coldStreak} straight. One manager is rolling. The other is desperate to stop the bleeding.` }); }
            ...
            if (aHistory.g >= 2 && bHistory.g >= 2 && maxPpg > 0) { const averagePpg = (aHistory.ppg + bHistory.ppg) / 2; if (averagePpg >= maxPpg * 0.88) stories.push({ p: 5, label: '💣 Shootout Alert', text: `${A.name} and ${B.name} are bringing serious firepower into this one. If both lineups hit, the scoreboard could get ridiculous in a hurry.` }); }
            ...
            const odd = [{ history: aHistory, manager: A, rank: aRank }, { history: bHistory, manager: B, rank: bRank }].find((entry) => entry.history.g >= 3 && ((entry.rank <= 3 && entry.history.ppg < medianPpg) || (entry.rank >= st.length - 2 && entry.history.ppg > medianPpg))); if (odd) stories.push({ p: 4, label: '🎯 Prove-It Game', text: `${odd.manager.name}'s record and scoring profile are telling two completely different stories. Week ${displayWeek} is another chance to prove whether this manager is real or living on borrowed time.` });
        }
        return [gameOfWeek, ...stories.sort((a, b) => b.p - a.p).slice(0, 3)].filter(Boolean);
    };
    $: stories = build();
</script>
<style>.box{width:95%;max-width:900px;margin:-12px auto 24px;display:grid;gap:10px}.story{padding:14px 16px;border:1px solid var(--ccc);border-radius:14px;background:var(--fff);box-shadow:0 4px 14px rgba(0,0,0,.06)}.label{font-size:.72rem;font-weight:850;letter-spacing:.5px;text-transform:uppercase;opacity:.65;margin-bottom:5px}.text{font-size:.9rem;line-height:1.5;font-weight:600}.text :global(strong){font-weight:900}@media(max-width:600px){.box{width:100%}.text{font-size:.82rem}}</style>
{#if stories.length}<section class="box">{#each stories as story}<div class="story"><div class="label">{story.label}</div><div class="text">{@html emphasizeManagers(story.text)}</div></div>{/each}</section>{/if}
