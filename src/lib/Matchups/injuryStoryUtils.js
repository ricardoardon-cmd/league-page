...
const playerName = (player, id) => {
    ...
    if (position === 'DEF') {
        return last || first || String(id).toUpperCase();
    }
...
const meaningfulStatus = (player) => {
    ...
    if (
        statusLower.includes('injured reserve') ||
        statusLower === 'ir' ||
        statusLower.includes('physically unable') ||
        statusLower === 'pup'
    ) {
        return status;
    }
...
const describe = (injury) => {
    ...
    if (injury.practice) {
        ...
        if (practice.includes('did not') || practice === 'dnp') {
            details.push('and did not practice');
        } else if (practice.includes('limited')) {
            details.push('and was limited in practice');
        }
    }

    if (!details.length) return '';
    return `${injury.name} is ${details.join(' ')}`;
};

export const matchupInjurySentence = (
    firstEntry,
    secondEntry,
    players = {},
    week,
    ...
    const injuries = [
        first ? { ...first, teamName: firstTeamName } : null,
        second ? { ...second, teamName: secondTeamName } : null
    ]
        .filter(Boolean)
    ...
    const clauses = injuries.map((injury) => {
        const description = describe(injury);
    ...
    if (phase === 'live') {
        return ` Sleeper injury context: ${clauses.join('; ')}.`;
    }

    if (phase === 'postgame') {
        return ` Injury context from Sleeper: ${clauses.join('; ')}.`;
    }
